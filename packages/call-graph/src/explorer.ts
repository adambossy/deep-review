...
function initExplorer(root, NAMES, onNavigate) {
  ...
  /* External restore point for a history entry: rebuild the track from a
     ...
     A panel still on the track is moved back into place as it stands,
     scroll included, rather than replaced; only one that has since gone
     comes back through the same lookup a walk uses (so one the server
     rendered earlier is still there, or fetched again), and is then scrolled
     to the anchor it was originally opened for. The highlighted pair is
     rebuilt last from the same descriptors a live walk records — which
     brings the step's own destination mark back into view even in a reused
     panel the reader has since scrolled away from, since that mark is what
     the step *is*. */
  root.__restore = function (ids, newPos, anchors, link) {
    var pool = Array.prototype.slice.call(track.children);
    function reclaim(id) {
      for (var k = 0; k < pool.length; k++) if (pool[k].dataset.node === id) return pool.splice(k, 1)[0];
      return null;
    }
    var panels = [];
    return ids.reduce(function (chain, id, i) {
      return chain.then(function () {
        var live = reclaim(id);
        if (live) { panels.push({ el: live, fresh: false }); return; }
        return panelFor(id).then(function (panel) {
          if (!panel) return;
          panel.__anchor = anchors && anchors[i] ? anchors[i] : null;
          panels.push({ el: panel, fresh: true });
        });
      });
      ...
      for (var f = 0; f < panels.length; f++) {
        if (!panels[f].fresh) continue;
        var el = anchorEl(panels[f].el, panels[f].el.__anchor);
        if (el) revealInPanel(panels[f].el, el);
      }
